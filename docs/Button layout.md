...
- `ButtonLayoutProfileCatalog` 提供稳定 ID 的 Xbox/Nintendo 初始配置以及增删改查。
...
4. 若最终为 `null` 或引用无效，则不管理硬件。

应用覆盖只覆盖 Button Layout 自身，不会复制或改变其他控制项。删除一个 Button Layout profile 后，预设和应用中的失效引用会在同一操作内被清理为 `null`。
...
Button Layout Quick Settings Tile 使用 `icon/sports_esports_24dp_E3E3E3_FILL1_wght400_GRAD0_opsz24.svg` 转换后的 Android Vector 图标。Tile 始终为 Active（配置库为空时为 Unavailable），短按按 catalog 顺序循环 Xbox、Nintendo 和全部用户 profile，末尾回到第一项；长按复用 Tile 选择弹窗，只显示当前 catalog 的 profile，不提供关闭项。Tile 选择会作为最高优先级覆盖应用/preset 自动化。
